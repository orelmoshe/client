(function (global, factory) {
  ...
  function cleanWKT(wkt) {
    ...
    if (
      !wkt.lat_ts &&
      wkt.lat1 &&
      (wkt.projName === 'Stereographic_South_Pole' || wkt.projName === 'Polar Stereographic (variant B)')
    ) {
            ...
            (1 +
              (als / 6) *
                (1 -
                  t +
                  c +
                  (als / 20) * (5 - 18 * t + ts + 14 * c - 58 * t * c + (als / 42) * (61 + 179 * ts - ts * t - 479 * t))))) +
        this.x0;
                ...
                (1 +
                  (als / 12) *
                    (5 -
                      ...
                      4 * cs +
                      (als / 30) *
                        (61 + ts - 58 * t + 270 * c - 330 * t * c + (als / 56) * (1385 + 543 * ts - ts * t - 3111 * t)))))) +
        this.y0;
    }
            ...
            (1 -
              (ds / 12) *
                (5 +
                  ...
                  4 * cs -
                  (ds / 30) *
                    (61 + 90 * t - 252 * c * t + 45 * ts + 46 * c - (ds / 56) * (1385 + 3633 * t + 4095 * ts + 1574 * ts * t))));
              ...
              (1 -
                (ds / 6) *
                  (1 +
                    ...
                    c -
                    (ds / 20) *
                      (5 + 28 * t + 24 * ts + 8 * c * t + 6 * c - (ds / 42) * (61 + 662 * t + 1320 * ts + 720 * ts * t))))) /
              cos_phi,
        );
      } else {
        lat = HALF_PI * sign(y);
  ...
  function forward$5(p) {
    ...
    p.y =
      2 * Math.atan(this.K * Math.pow(Math.tan(0.5 * lat + FORTPI), this.C) * srat(this.e * Math.sin(lat), this.ratexp)) -
      HALF_PI;
    p.x = this.C * lon;
    return p;
  }
  ...
  function inverse$6(p) {
    ...
    if (this.sphere) {
      ...
      if (Math.abs(this.coslat0) < EPSLN) {
        ...
        lon = adjust_lon(
          this.long0 + Math.atan2(p.x * Math.sin(c), rh * this.coslat0 * Math.cos(c) - p.y * this.sinlat0 * Math.sin(c)),
        );
      }
      p.x = lon;
      p.y = lat;
      ...
      if (Math.abs(this.coslat0) <= EPSLN) {
        ...
        if (rh <= EPSLN) {
          ...
          lon = adjust_lon(
            this.long0 + Math.atan2(p.x * Math.sin(ce), rh * this.cosX0 * Math.cos(ce) - p.y * this.sinX0 * Math.sin(ce)),
          );
        }
  ...
  function forward$23(p) {
    ...
    var msq = m * m;
    var con =
      (Math.PI * this.R * (al * (g - msq) + Math.sqrt(asq * (g - msq) * (g - msq) - (msq + asq) * (gsq - msq)))) / (msq + asq);
    if (dlon < 0) {
      con = -con;
    }
    x = this.x0 + con;
